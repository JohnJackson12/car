...
class SettingsActivity : Activity() {
    ...
    private val SOUND_FILE_REQ = 1002
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        body.addView(Ui.button(this, "Test sound") { FeedbackSounds.playAck(this, config.feedbackSound, config.feedbackVolume, config.feedbackCustomPath) })
        body.addView(Ui.button(this, "Choose custom sound file\u2026") { startActivityForResult(Intent(this, FolderBrowserActivity::class.java).putExtra(FolderBrowserActivity.EXTRA_FILE_MODE, true), SOUND_FILE_REQ) })
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        } else if (requestCode == SOUND_FILE_REQ && resultCode == Activity.RESULT_OK) {
            val path = data?.getStringExtra(FolderBrowserActivity.EXTRA_PATH) ?: return
            config.feedbackCustomPath = path
            config.feedbackSound = "custom"
            Ui.toast(this, "Custom sound set: ${java.io.File(path).name}")
        }
    }
    ...
    private fun accentColorRow(): LinearLayout {
        val swatchHexes = listOf("#e0a94b", "#5b9dd9", "#6fd6c4", "#8fbf5c", "#d16a6a", "#b57ee0")
        val row = Ui.row(this)
        fun refresh() {
            row.removeAllViews()
            for (hex in swatchHexes) {
                val v = View(this).apply {
                    layoutParams = LinearLayout.LayoutParams(Ui.dp(this@SettingsActivity, 32), Ui.dp(this@SettingsActivity, 32)).apply { setMargins(Ui.dp(this@SettingsActivity,4),0,Ui.dp(this@SettingsActivity,4),0) }
                    setBackgroundColor(Color.parseColor(hex))
                    if (config.accentColor.equals(hex, ignoreCase = true)) setPadding(4,4,4,4)
                    setOnClickListener { config.accentColor = hex; refresh() }
                }
                row.addView(v)
            }
            row.addView(Ui.iconButton(this, "+", 18f) {
                Ui.prompt(this, "Custom accent color", config.accentColor.ifEmpty { "#e0a94b" }, "#RRGGBB") { hex ->
                    try { Color.parseColor(hex); config.accentColor = hex; refresh() } catch (e: Exception) { Ui.toast(this, "Not a valid color - use the format #RRGGBB") }
                }
            })
            row.addView(Ui.button(this, "Reset") { config.accentColor = ""; refresh() })
        }
        refresh()
        return labeled("Accent color", row)
    }
