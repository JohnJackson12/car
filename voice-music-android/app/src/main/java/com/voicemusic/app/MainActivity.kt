...
class MainActivity : Activity(), PlaybackService.UiListener {
    ...
    private enum class Kind { SONGS, ALBUMS, ALBUM_DETAIL, ARTISTS, ARTIST_DETAIL, GENRES, GENRE_DETAIL,
        FOLDERS, FOLDER_DETAIL, PLAYLISTS, PLAYLIST_DETAIL, FAVORITES, RECENT, MOST_PLAYED, QUEUE }
    ...
    private fun micIndicatorView(): TextView {
        micIndicator = Ui.textView(this, "\uD83C\uDF99", 18f).apply {
    ...
    private fun buildRail() {
        ...
        sectionRail.addView(railButton("Recent", Kind.RECENT))
    ...
    private fun buildNowPlayingBar(): LinearLayout {
        ...
        npShuffle = Ui.iconButton(this, "\uD83D\uDD00", 18f) { svc?.engine?.let { it.setShuffle(!it.shuffleEnabled); refreshNowPlaying() } }
        npRepeat = Ui.iconButton(this, "\uD83D\uDD01", 18f) { svc?.engine?.cycleRepeat(); refreshNowPlaying() }
        val prev = Ui.iconButton(this, "\u23EE", 24f) { svc?.engine?.previous() }
        npPlayPause = Ui.iconButton(this, "\u25B6", 26f) { togglePlayPause() }
        val next = Ui.iconButton(this, "\u23ED", 24f) { svc?.engine?.next() }
        val menu = Ui.iconButton(this, "\u22EE", 20f) { showSongMenu(svc?.engine?.currentSong() ?: return@iconButton) }
        val topRow = Ui.row(this, npArt, textCol, npShuffle, prev, npPlayPause, next, npRepeat, menu)
        ...
        return Ui.col(this, Ui.divider(this), topRow, seekRow).apply { setBackgroundColor(Color.rgb(24, 24, 30)) }
    }
    ...
    private fun enableMultiSelect(songs: List<Song>) {
        ...
        listView.setMultiChoiceModeListener(object : android.widget.AbsListView.MultiChoiceModeListener {
            ...
            override fun onCreateActionMode(mode: android.view.ActionMode, menu: android.view.Menu): Boolean {
                ...
                menu.add(0, 3, 2, "Add to playlist")
    ...
    private fun showSongMenu(song: Song) {
        val items = mutableListOf(
            ...
            if (song.approved) "Un-approve" else "Approve", "Edit tags", "Trim", "Add to playlist",
            "Move to folder", "Copy to folder", "Song info", "Delete"
        )
        android.app.AlertDialog.Builder(this).setTitle(song.title).setItems(items.toTypedArray()) { _, which ->
            when (items[which]) {
                "Play" -> { val list = currentSongList(); val idx = list.indexOfFirst { it.id == song.id }.coerceAtLeast(0); playFromList(list.ifEmpty { listOf(song) }, idx) }
    ...
    private fun refreshNowPlaying() {
        ...
        npRepeat.text = when (e.repeatMode) { "one" -> "\uD83D\uDD02"; "all" -> "\uD83D\uDD01"; else -> "\uD83D\uDD01" }
