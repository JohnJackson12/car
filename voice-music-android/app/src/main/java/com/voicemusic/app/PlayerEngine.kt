...
class PlayerEngine(private val ctx: Context, private val library: Library, private val config: Config) {
    ...
    /**
     * Trashes the currently playing song safely: advances off it first (so the file isn't open
     * while it's being moved), then trashes it, then drops it from the queue. Matches the PC app's
     * behavior of moving straight on to the next song after a voice "delete".
     */
    fun deleteCurrentSong(library: Library): Pair<Boolean, Boolean> {
        val s = currentSong() ?: return Pair(false, false)
        val id = s.id
        if (queue.size > 1) next() else stopAndRelease()
